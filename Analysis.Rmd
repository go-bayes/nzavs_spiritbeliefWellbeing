...
Inference holds but VIFS years x beliefs when years are centered.

```{r model centering years}
library(lme4)
mod.1a <-lmer (PWI ~  1 +  yearsC * Beliefs +  Age.10yrs.C + Deprivation.S + Edu.S  + Employed + EthnicCats + Male  + Partner  + Pol.Orient.S + Urban + (1|Id), data= dfben, na.action = na.omit)
mod.2a <-lmer (LIFESAT ~ 1 + yearsC * Beliefs+ Age.10yrs.C + Deprivation.S + Edu.S  + Employed + EthnicCats + Male  + Partner  + Pol.Orient.S + Urban + (1|Id), data= dfben, na.action = na.omit)
summary(mod.1a)
summary(mod.2a)
# identical result
...
# address reviewer worry about deprivation and partner being correlated
m<-lmer (NZdep ~ Partner + (1|Id), data= dfben, na.action = na.omit)
tab_model(m)
model_parameters(m)
```

Model is robust without covariates

```{r robust to years and beliefs only}
library(lme4)
mod.1aa <-lmer (PWI ~  1 +  years * Beliefs + (1|Id), data= dfben, na.action = na.omit)
mod.2aa <-lmer (LIFESAT ~ 1 + years * Beliefs + (1|Id), data= dfben, na.action = na.omit)
)
# identical result
...
# get estimates for PWI from above
p233 <- plot(mp133) + labs(title ="Coefficients Personal Wellbeing (imputed)")
```
