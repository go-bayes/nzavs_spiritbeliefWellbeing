...
<!-- #demographic_table_latex(d_3) # note the furniture package conflicts with the table1 package.  -->
<!-- ``` -->
...
In the reported analyses, we included covariates to adjust for potential confounding effects from usual suspect factors associated with our two measures of well-being (Personal Well-Being, Life Satisfaction). Where $Z$ is a third variable, $X$ is the predictor, and $Y$ is the outcome, there are at least four types of confounding that might arise: 
...
We report our usual-suspect analysis below, which does not change the inference in this study. However, the third and fourth type of confounding illustrate how conditioning on a covariates may confound by *opening* a relationship between the $X$ and $Y$ indicators in a regression analysis. Such confounding is particularly worrying in the case of longitudinal research.  The second type of counfounding can mask a relationship. We do not wish to exclude third variable explanations of the kind that arise from the first type of confounding. Indeed, we are careful to state in the main text that our study cannot infer a causal relationship between spirit beliefs and well-being. Rather, the contribution of the study comes as an inference about how the expected relationship between beliefs and well-being changes between those who differ in such beliefs over time. By contrast, a causal inference requires testing specific mechanistic theory. This is a matter for future research. Finally, the usual suspect analysis that we report below does not affect inference. We find that the relationships that we observe in the simple analysis also hold there. 
...
plot(mod.2_b, pars ="sd_") # requires stronger priors
```
...
### Test of the Bayesian saturacted model


```{r setmodelformulaSAT}
forumula_bayesPWI_SATUR <- bf(PWI ~  yearsC * Beliefs +  Age_in_Decades_C + Deprivation_S + Education_S  + Employed + Ethnic_Categories + Male  + Has_Partner  + Political_Conservativism_S + Urban + (1+ yearsC|Id))
forumula_bayesLIFESAT_SATUR <- bf(LIFESAT ~  yearsC * Beliefs +  Age_in_Decades_C + Deprivation_S + Education_S  + Employed + Ethnic_Categories + Male  + Has_Partner  + Political_Conservativism_S + Urban + (1+ yearsC|Id))
```
```{r pwi_bayesmodelSAT}
# PWI
