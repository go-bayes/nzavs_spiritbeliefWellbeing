...
This supplement offers a step-by-step analysis for the "Longitudinal
Evidence for Growth in Subjective Well-being from Spiritual Beliefs."
The functions and other materials for this analysis can be found at the
...
# d2<-data_clean_spirit_wellbeing(pa,0) # d = dataframe, 3 = number of waves
# table(d2$Wave) 


# function to clean data, specify number of waves as 3 
d_3<-data_clean_spirit_wellbeing(d,9) # d = dataframe, 3 = number of waves
...
<!-- ## How many waves to select? We are using 10 years -->

<!-- We selected a minimum of three waves because we want to recover a -->

<!-- longitudinal sample in which a slope for the effect of the years -->

<!-- $\times$ beliefs effect might be recovered. However, we needed to -->

<!-- consider whether our selection of three waves was sensitive to that -->

<!-- degree of researcher freedom. So we also prepared 4-8 years of data -->

<!-- frames for separate analyses. -->
...
<!-- The analyses for these waves is described below. These analyses reveal -->

<!-- that the inference of the paper is not sensitive to the threshold number -->

<!-- of waves for participant selection. Which is to say, we recover the same -->

<!-- inference with an -wave selection criterion. -->
...
tidy_ggdag <- dagify(
  WB ~ belief + age_within + age_between + partner + nzdep + urban + male + pols + empl, # what might affect wellbeing?
  WB ~~ partner, # bi-causal path
  belief ~ age_within  + age_between  + male + ethn, # predictors of beliefs
  partner ~ nzdep + age_within  + age_between + belief,  # predictors of having a partner
  nzdep ~ empl,  # predictor of deprivation
  pols ~ age_within  + age_between  + empl + ethn, # predictor of political orientation
  empl ~ nzdep + edu + ethn + age_within  + age_between, # predictor of employment
  exposure =  "belief",
  outcome =   "WB")%>%
  tidy_dagitty() + theme_dag_gray()
